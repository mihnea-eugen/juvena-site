import Link from "next/link";
import Image from "next/image";

const LINKS_TRATAMENTE = [
  { label: "Acid Hialuronic", href: "/dermatologie-estetica/injectari-acid-hialuronic/" },
  { label: "Toxină Botulinică", href: "/dermatologie-estetica/injectari-toxina-botulinica/" },
  { label: "Skinbooster", href: "/dermatologie-estetica/injectari-skinbooster/" },
  { label: "Biostimulatori Colagen", href: "/dermatologie-estetica/injectari-biostimulatori-colagen/" },
  { label: "Dermatoscopie", href: "/dermatologie-medicala/dermatoscopie/" },
  { label: "Peeling Chimic", href: "/tratamente-faciale/peeling-chimic/" },
];

const LINKS_CLINIC = [
  { label: "Despre noi", href: "/despre-noi/" },
  { label: "Medicii noștri", href: "/medici/" },
  { label: "Prețuri", href: "/preturi/" },
  { label: "Contact & Programări", href: "/contact/" },
];

export default function Footer() {
  const year = new Date().getFullYear();

  return (
    <footer className="bg-[var(--dark)] text-cream/70">
      {/* Main footer */}
      <div className="container py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          {/* Brand */}
          <div className="lg:col-span-1">
            <Link href="/" className="flex items-center gap-3 mb-5">
              <Image
                src="/images/logo.png"
                alt="Juvena Clinic Timișoara"
                width={130}
                height={121}
                className="h-10 w-auto"
              />
            </Link>
            <p className="text-sm leading-relaxed text-cream/50 mb-6">
              Clinică de dermatologie estetică și medicală în Timișoara.
              Medici specializați, rezultate naturale.
            </p>
            {/* Social */}
            <div className="flex gap-3">
              <a
                href="https://www.instagram.com/juvena.timisoara"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Instagram Juvena Timișoara"
                className="w-9 h-9 border border-white/20 flex items-center justify-center hover:border-gold hover:text-gold transition-colors"
              >
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                </svg>
              </a>
              <a
                href="https://www.facebook.com/juvena.timisoara"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Facebook Juvena Timișoara"
                className="w-9 h-9 border border-white/20 flex items-center justify-center hover:border-gold hover:text-gold transition-colors"
              >
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                </svg>
              </a>
            </div>
          </div>

          {/* Clinica */}
          <div>
            <h3 className="text-cream text-xs tracking-widest uppercase mb-5 font-medium">
              Clinica
            </h3>
            <ul className="space-y-3">
              {LINKS_CLINIC.map((l) => (
                <li key={l.href}>
                  <Link
                    href={l.href}
                    className="text-sm hover:text-gold transition-colors"
                  >
                    {l.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Tratamente */}
          <div>
            <h3 className="text-cream text-xs tracking-widest uppercase mb-5 font-medium">
              Tratamente
            </h3>
            <ul className="space-y-3">
              {LINKS_TRATAMENTE.map((l) => (
                <li key={l.href}>
                  <Link
                    href={l.href}
                    className="text-sm hover:text-gold transition-colors"
                  >
                    {l.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h3 className="text-cream text-xs tracking-widest uppercase mb-5 font-medium">
              Contact
            </h3>
            <address className="not-italic space-y-3 text-sm">
              <div className="flex gap-2">
                <svg className="w-4 h-4 text-gold shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
                <span className="text-cream/60">Strada Arieș nr 3<br />Timișoara, Timiș<br />România</span>
              </div>
              <div className="flex gap-2 items-center">
                <svg className="w-4 h-4 text-gold shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                </svg>
                <a href="tel:+40767203877" className="hover:text-gold transition-colors text-cream/60">
                  0767 203 877
                </a>
              </div>
              <div className="pt-2">
                <Link href="/contact/" className="btn-outline text-xs py-2 px-4">
                  Programare online
                </Link>
              </div>
            </address>
          </div>
        </div>
      </div>

      {/* Bottom bar */}
      <div className="border-t border-white/10">
        <div className="container py-5 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-cream/40">
          <span>© {year} Juvena Timișoara. Toate drepturile rezervate.</span>
          <div className="flex gap-5">
            <Link href="/politica-confidentialitate/" className="hover:text-cream/70 transition-colors">
              Politică confidențialitate
            </Link>
            <Link href="/termeni-conditii/" className="hover:text-cream/70 transition-colors">
              Termeni & condiții
            </Link>
          </div>
        </div>
      </div>
    </footer>
  );
}
